...
let showQustion = () => {
    ...
    content.innerHTML += "<p class='expText'>הקלידו את תשובותכם:</p>"
    content.innerHTML += "<input type='text' id='answer' dir='ltr' /><br /><button onclick='checkAnswer()'>בדקו את תשובתכם</button>"
...
let showPlace = (doneAll) => {
    if(doneAll) // done all qustions
        ...
        content.innerHTML += `<p class='expText'>לאחר שהגעתם למקום צלמו סלפי עם חברי הקבוצה ושלחו למספר<br />${phoneNumbers[Math.floor(myRoute / 2 + 0.5) - 1]}<br />את התמונה בווטסאפ.<br /><u>לאחר אישור התמונה, ישלח לכם קוד לשאלה הבאה! בהצלחה ♥</u><br /><br />הכניסו את הקוד:</p>`
        content.innerHTML += "<input type='text' id='code' dir='ltr' /><br /><button onclick='checkQustionCode()'>עברו לשאלה הבאה</button>"
    }
}
...
let goToSchool = () => {
    ...
    content.innerHTML += "<p class='title' style='color:#8d2d9e'>הגעתם לסוף המירוץ, עליכם להגיע לבית הספר כמה שיותר מהר!<br />בהצלחה ♥</p>"
